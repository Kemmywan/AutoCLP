...
# llm_1: Turn dialogue into basic summary 
llm_1 = OpenAIChatCompletionClient(
    model="gpt-4.1",
    api_key="sk-Td0lvL6RVNsep6BWAeDf07E9Eb0742C5B5F442C8F63fFe1b",
    base_url="https://aihubmix.com/v1",
)

# llm_2: Read summary and generate the exam appointment
llm_2 = OpenAIChatCompletionClient(
    model="gpt-4.1",
    api_key="sk-Td0lvL6RVNsep6BWAeDf07E9Eb0742C5B5F442C8F63fFe1b",
    base_url="https://aihubmix.com/v1",
)

# llm_3: Read the exam result and use it to supple the summary
llm_3 = OpenAIChatCompletionClient(
    model="gpt-4.1",
    api_key="sk-Td0lvL6RVNsep6BWAeDf07E9Eb0742C5B5F442C8F63fFe1b",
    base_url="https://aihubmix.com/v1",
)

# llm_4: Read the summary and generate a dianostic report
llm_4 = OpenAIChatCompletionClient(
    model="gpt-4.1",
    api_key="sk-Td0lvL6RVNsep6BWAeDf07E9Eb0742C5B5F442C8F63fFe1b",
    base_url="https://aihubmix.com/v1",
)

# llm_5: Read the diagnostic report and offer advice and treatment method
llm_5 = OpenAIChatCompletionClient(
    model="gpt-4.1",
    api_key="sk-Td0lvL6RVNsep6BWAeDf07E9Eb0742C5B5F442C8F63fFe1b",
    base_url="https://aihubmix.com/v1",
)

# llm_test: Used to generate the exam result (for test)
llm_test = OpenAIChatCompletionClient(
    model="gpt-4.1",
    api_key="sk-Td0lvL6RVNsep6BWAeDf07E9Eb0742C5B5F442C8F63fFe1b",
    base_url="https://aihubmix.com/v1",
)
...
SYSTEM_MESSAGE_3 = """你是一个专业的医疗信息补充AI助手。
...
SYSTEM_MESSAGE_4 = """你是一个专业的病状诊断AI助手。
你的任务是读取一份json格式的初步问诊笔记，
...
SYSTEM_MESSAGE_5 = """你是一个专业的疾病治疗康复AI助手。
...
async def main() -> None:
    ...
    agent_5 = AssistantAgent(
        name="Agent5",
        system_message=SYSTEM_MESSAGE_5,
        model_client=llm_5
    )
    ...
    agent_test = AssistantAgent(
        ...
        model_client=llm_test
    )

    with open("task.txt", "r", encoding="utf-8") as f:
        TASK=f.read()

    print("agent_1 is generating summary...")
    result = await agent_1.run(task=TASK)
    summary = result.messages[1].to_text()
    print("agent_1 give summary:")
    print(summary)
    print("="*16)

    print("agent_2 is generating exam appointment...")
    result_2 = await agent_2.run(task=summary)
    exam_appointment = result_2.messages[1].to_text()
    print("agent_2 give exam appointment:")
    print(exam_appointment)
    print("="*16)

    exam_result = await agent_test.run(task=exam_appointment+summary)
    exam_result_text = exam_result.messages[1].to_text()
    print("exam result (for test):")
    print(exam_result_text)
    print("="*16)

    print("agent_3 is supplementing summary...")
    result_3 = await agent_3.run(task=f"{summary}\n{exam_result_text}")
    supplemented_summary = result_3.messages[1].to_text()
    print("agent_3 give supplemented summary:")
    print(supplemented_summary)
    print("="*16)

    print("agent_4 is generating diagnostic report...")
    result_4 = await agent_4.run(task=supplemented_summary)
    diagnostic_report = result_4.messages[1].to_text()
    print("agent_4 give diagnostic report:")
    print(diagnostic_report)
    print("="*16)

    print("agent_5 is generating advice and treatment...")
    result_5 = await agent_5.run(task=diagnostic_report)
    advice = result_5.messages[1].to_text()
    print("agent_5 give advice and treatment:")
    print(advice)
